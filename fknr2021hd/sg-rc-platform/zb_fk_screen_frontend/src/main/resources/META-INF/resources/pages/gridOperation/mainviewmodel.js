define(["RESTClient","echarts","axios"], function(RestClient,echarts,axios) {
    var PageViewModel = function(params) {
        ...
        self.day=function (flag) {
            self.WARNINGLEVEL1("0");//一级
            self.WARNINGLEVEL2("0");//二级
            self.WARNINGLEVEL3("0");//三级
            self.WARNINGLEVEL4("0");//四级
            self.WARNINGLEVEL5("0");//五级
            self.WARNINGLEVEL6("0");//六级
            self.WARNINGLEVEL7("0");//七级
            self.WARNINGLEVEL8("0");//八级
            self.WARNINGLEVEL("0");//电网风险总数
            self.WORK_PLAN_WARN_CNT("0");//关联作业总数
            self.isShow(true)
        ...
        function riskGridWarnList() {
            ...
            var iptRep3 = new RegExp("[-`~!！@$^&%*+=|\\\\{}''\\[\\]<>《》，。,.:;：；/￥……——{}\"‘”“'_()?？·（）【】’#、]")
            if(iptRep.test(self.Name().trim()) || iptRep2.test(self.number().trim()) || iptRep3.test(self.Unit().trim())) {
...
                return;
            ...
            // 输入框长度校验
            if(self.Name().trim().length > 100 || self.number().trim().length > 100 || self.Unit().trim().length > 100) {
                cube.indicate("输入框内容长度不能超过100！");
                return;
            }
            // 时间空格校验
