	...
	var PageViewModel = function(params) {
		...
		self.eventChangeByEventType = function(e) {
			if (self.selectIdByEventType() == '1') {
				var params = {"type":1,"fldw":'fl'}
				axios.post(cube.gatewayURL2 + "riskAccidentAnalysis/accidentCountByMonthTwo",params).then(function(res) {
					if (true) {
		...
		self.columns = [
			...
			{name: "STARTTIME", width:"10%", caption: "计划开始时间" , editorType: "TextEditor",align : "center" },
			{name: "ENDTIME", width:"10%", caption: "计划结束时间" , editorType: "TextEditor",align : "center" },
						...
						if(data[key]) {
							switch(key){
								case 'personalAccident':
									Number.push({name:'人身事件',value:data[key]})
									break;
								case 'equipAccident':
									Number.push({name:'设备事件',value:data[key]})
									break;
								case 'informAccident':
									Number.push({name:'网络信息事件',value:data[key]})
									break;
								case 'powerGridAccident':
									Number.push({name:'电网事件',value:data[key]})
									break;
							}
						}
					}
		...
		function MonthlyStatistics(){
			// var params = {"type":0}
			axios.post(cube.gatewayURL2 + "riskAccidentAnalysis/accidentCountTwo").then(function(res) {
		...
		function RiskList(searchParams) {
            ...
            if(iptRep.test(self.voltageCapacity().trim()) || iptRep.test(self.gridcorp().trim())) {
                cube.indicate("输入框含违法字符，请重新输入！");
                return;
            }
			// 输入框空格校验
			if(self.voltageCapacity().trim().length !== self.voltageCapacity().length) {
				cube.indicate("输入框内容不能全空格或首尾有空格！");
				return;
			}
			if(self.gridcorp().trim().length !== self.gridcorp().length) {
				cube.indicate("输入框内容不能全空格或首尾有空格！");
				return;
			}
            // 输入框长度校验
            if(self.voltageCapacity().trim().length > 100 || self.gridcorp().trim().length > 100) {
                cube.indicate("输入框内容长度不能超过100！");
                return;
            }
			// 时间空格校验
			// if(self.startTime().trim().length !== self.startTime().length) {
			...
			// }
            // 时间格式校验
            if(self.startTime().trim() || self.endTime().trim()) {
                var begin_r = self.startTime().trim() ? self.startTime().trim().match(/^(\d{4})(-)(\d{2})(-)(\d{2})\s(\d{2})(:)(\d{2})(:)(\d{2})*/) : 1;
...
                    return;
                }else if (self.startTime().trim() && self.endTime().trim()) {
                    if (self.startTime().trim() > self.endTime().trim()) {
                        cube.indicate("计划开始时间不能大于计划结束时间");
                        return;
                    }
                }
            }
			...
			axios.post(cube.gatewayURL2 + "riskAccidentAnalysis/equipAccidentListTwo",params).then(function(res) {
		...
		function Grids() {
			// 输入框内容校验
			var iptRep = new RegExp("[-`~!！@$^&%*+=|\\\\{}''\\[\\]<>《》，。,.:;：；/￥……——{}\"‘”“'_()?？·（）【】’#、]")
            if(iptRep.test(self.voltageCapacity().trim())) {
                cube.indicate("输入框含违法字符，请重新输入！");
                return;
            }
			// 输入框空格校验
            ...
            // 输入框长度校验
            if(self.voltageCapacity().trim().length > 100) {
                cube.indicate("输入框内容长度不能超过100！");
                return;
            }
			// 时间空格校验
...
            }
			...
			axios.post(cube.gatewayURL2 + "riskAccidentAnalysis/powerGridAccidentListTwo",params).then(function(res) {
				if (res.data.successful) {
...
            }
			...
			axios.post(cube.gatewayURL2 + "riskAccidentAnalysis/personalAccidentListTwo",params).then(function(res) {
				if (res.data.successful) {
...
            }
			...
			axios.post(cube.gatewayURL2 + "riskAccidentAnalysis/informAccidentListTwo",params).then(function(res) {
				if (res.data.successful) {
