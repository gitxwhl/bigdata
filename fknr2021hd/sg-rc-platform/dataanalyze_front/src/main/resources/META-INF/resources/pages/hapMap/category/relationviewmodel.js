...
function newadd(searchParams) {
    ...
    if(!self.nodeName().trim()){
        cube.indicate("请输入描述");
        return
    }
    if(!self.runit().trim()){
        cube.indicate("请选择源类别");
        return
    }
    if(!self.tunit().trim()){
        cube.indicate("请选择目标类别");
        return
    }
    if(!self.aunit().trim()){
    ...
    if(iptRep.test(self.genrelas().trim()) || iptRep2.test(self.nodeName().trim())) {
    ...
    if(self.nodeName().trim().length !== self.nodeName().length) {
        cube.indicate("输入框内容不能全空格或首尾有空格！");
        return;
    }
    // 输入框长度校验
    ...
    if(self.nodeName().trim().length > 200) {
        cube.indicate("描述长度不能超过200！");
        return;
    }
    var params = {
...
self.targetUnit=ko.observableArray([
    {id:'2',text:'外包单位'},
    {id:'3',text:'人员'},
    {id:'4',text:'风险'},
    {id:'5',text:'隐患'},
    ...
    function initSearch() {
        ...
        relationshipmbxz=function relationshipmbxz(){
            ...
            importExcel=function(obj){
                ...
                console.log($("#articleImageFile")[0].files[0])
                ...
                var params = formData
                console.log(params)
                axios.post(cube.gatewayURL2 + "fileLead/fileRelationLead",params,{headers: {'Content-Type':'multipart/form-data'}}).then(function(res) {
                    if (res.data.successful) {
                        cube.indicate("导入成功");
                        RiskList(self.pageIndex())
                    } else {
