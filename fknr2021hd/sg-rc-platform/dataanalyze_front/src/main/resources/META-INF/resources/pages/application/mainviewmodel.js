define(["d3","RESTClient", "echarts", "axios"], function(d3,RestClient, echarts, axios) {
	var PageViewModel = function(params) {
        ...
        self.search = function(){
			...
			if(iptRep.test(self.searchKeywords().trim())) {
				cube.indicate("输入框含违法字符，请重新输入！");
                return;
			}
			// 输入框空格校验
			if(self.searchKeywords().trim().length !== self.searchKeywords().length) {
				cube.indicate("输入框内容不能全空格或首尾有空格！");
				return;
			}
			// 输入框长度校验
			if(self.searchKeywords().trim().length > 100) {
				cube.indicate("输入框内容长度不能超过100！");
                return;
			}
			var params = {
				"instanceName": self.searchKeywords()
			}
			self.isShow(true)
            axios.post(cube.gatewayURL2 + "atlasInformation/getGplotName", params).then(function (res) {
				// self.searchKeywords('')
                ...
                //   console.log(links)
					if(res.data.resultValue.branch===null){
						cube.indicate("没有该名称的实例");
					}
				linkq=res.data.resultValue.principalLine.concat(res.data.resultValue.branch)
		...
		links.forEach(function (link,i) {
			...
			link.source = nodes[link.source] || (nodes[link.source] = { name: link.source });
			link.target = nodes[link.target] || (nodes[i] = { name: link.target });
		});
		var width = $("#d3").width();
			height = $("#d3").height();
