define([ 'axios',"md5","./app","sammy" ], function(axios,md5) {
    // axios.default.headers['Content-Type']='applcation/json;charset=utf-8'
    ...
    axios.interceptors.request.use(config => {
        ...
        if('get'==p_method){
            ...
            if(pdata) {
                ...
                params = urlt + "userIdc32ad1415f6c89fee76d8457c31efb4b";
            }
        }else {
...
//	        return md5.hex(signString).toUpperCase();
    ...
    var PageViewModel = function() {
        ...
        axios.get(cube.gatewayURL3 + "app/menu/getMenuTree").then(function(res) {
            if (res.data.successful) {
                ...
                if(arr.length>0) {
                    ...
                    var menustr = ``
                    arr.forEach(ele=>{
                        if (ele.menu.name == '外包单位作业风险画像') {
                            menustr += `<div>
                                <div class="list hand" style="margin-top:10px;" datapage="outsourcing.main">外包单位作业风险画像</div>
                            </div>`
                        }else {
        ...
        // self.url = ko.observable('dataCleansing.main');
        // self.url = ko.observable('menuPage.main');
