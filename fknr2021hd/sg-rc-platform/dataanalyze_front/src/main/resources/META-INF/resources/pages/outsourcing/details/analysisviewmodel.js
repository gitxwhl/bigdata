			...
			axios.post(cube.gatewayURL2 + "accidentInformation/safetyAnalysis",params).then(function(res) {
				...
				if (res.data.successful) {
					...
					for(var i = 0;i<list4.length;i++){
						if(list4[i]['score'].length > 0) {
							let scoreObj = list4[i]['score'][0]
							let newScore = scoreObj.BASICINFO_POINTS || scoreObj.CERTIFICATE_PHOTO_POINTS || scoreObj.SAFELEARNING_POINTS || scoreObj.SAFETYTEST_POINTS
