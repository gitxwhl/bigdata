...
define([], function() {
    ...
    /**
     * 请求后台类型，可选post、get，默认为get，当ids不为null时为默认为post
     */
    me.type = "get";
    ...
    /**
     * 设置 {@link ids} 的值。
     * @param p_ids 一个数组对象 ["1","2"]
     */
    me.setIds = function(p_ids) {
        if (me.ids != p_ids) {
            me.ids = p_ids;
        }
    };
    ...
    me.exportExcel = function() {
    	if (me.baseUrl != null) {
            if (me.ids == null && me.type == "get") {
            	...
            	window.open(cube.mappath(me.baseUrl) + (cube.isEmpty(params) ? "" : "?" + params), "_parent");
            } else {
            	var __exportExcelFormFrame = $("#__exportExcelFormFrame");
		        if (__exportExcelFormFrame.length > 0) {
		        	__exportExcelFormFrame.remove();
            	}
		        
		        var iframe = $("<iframe id='__exportExcelFormFrame' name='__exportExcelFormFrame' style='display:none;position:absolute;main:-1000px;width:1px;height:1px'></iframe>");
        		$("body").append(iframe);
            	...
            	iframe.on("load", function(){
        			var content = "";
     			    ...
     			    if (cube.notEmpty(content)) {
     			    	cube.indicate("warning", cube.msg("EXPORT_FAIL"));
     			    }
        		})
            	
            	var form = $("#__exportExcelForm");
            	if (form.length == 0) {
            		form = $("<form>");
				    form.attr("id", "__exportExcelForm");
				    form.attr("style", "display:none");
				    form.attr("target", "__exportExcelFormFrame");
            	...
            	form.attr("action", me.baseUrl);  
			 
            	if (me.ids != null) {
            		var input = $("<input>");
     			    input.attr("type", "hidden");
     			    input.attr("name", "ids");
     			    input.attr("value", me.ids.join(","));
     			    form.append(input);
            	}
			    ...
			    if ($.isPlainObject(me.params)) {
	                for (var key in me.params) {
					    ...
					    input.attr("type", "hidden");
					    input.attr("name", key);
					    input.attr("value", me.params[key]);
					    form.append(input);
	                }
	            }
