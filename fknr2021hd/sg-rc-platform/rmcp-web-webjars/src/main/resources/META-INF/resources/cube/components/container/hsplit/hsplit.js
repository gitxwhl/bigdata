define([], function() {
	 ...
	 * 				self.contents = [{
	 * 									height:"200px",
	 *  								content: '第一行',//当content属性存在时优先显示其内容，否则显示templateOptions配置的页面
	 *  								templateOptions: {
	 ...
	 *  							{
	 *  								height:"200px",
	 *  								content: '第二行',//当content属性存在时优先显示其内容，否则显示templateOptions配置的页面
	 *  								templateOptions: {
	 *		    										   name: 'app_cubedemo.componentDemos.containerDemos.hsplit.Templ',
	 *		    										   params: {title: '第一行的模板'}
	 *		    									   }
	 ...
	 *  							{
	 *  								height:"200px",
	 *  								content: '第三行',//当content属性存在时优先显示其内容，否则显示templateOptions配置的页面
	 *  								templateOptions: {
	 *		    										   name: 'app_cubedemo.componentDemos.containerDemos.hsplit.Templ',
	 *		    										   params: {title: '第一行的模板'}
	 *		    									   }
	 ...
	 */
 	function ViewModel(params) {
		...
		/**
	     * 设置一个 Boolean 值，表示是否显示边框，值为true时显示边框。
	     * 
	     * @default true
	     */
		self.showBorder = true;
		...
		self._init = function (){
			...
			for (var i = 0; i < length; i++) {
				if (contents[i]) {
					...
					contents[i].height = cube.initComponentProperty(contents[i].height, "auto", 'obj');
		 ...
		 * @ignore
		 */
		var _mouseY = 0, _srcY = 0, contentHeight;
		self.mousedown = function(index, data, event) {
			_mouseY = event.clientY;
			contentHeight = self.contents()[index()].height;
			var _height = contentHeight();
			if(_height == "auto" || _height.indexOf("%") != -1){
				var target = (event.currentTarget) ? event.currentTarget : event.srcElement;
				_srcY = $(target).prev().height();
			}else{
				_srcY = parseInt(_height);
			}
		...
		self.mousemove = function() {
			 var offsetY = event.clientY - _mouseY;
			 contentHeight((_srcY + offsetY) + "px");
		};
	    ...
	    self.onloading = function(node, viewModel) {
	    	...
	    	pages.each(function(index,element){
	    		...
	    		var page = {
	    			   height: cube.obj($element.attr("height")),
	    ...
	    self.onload = function(node, viewModel) {
	    	...
	    	for (var i = _length; i < panels.length; i++) {
				cube.applyBindings($e.find(">.row-fluid>div.row-fluid").eq(i).children().eq(0).children(), viewModel);
	    	}
	    };
