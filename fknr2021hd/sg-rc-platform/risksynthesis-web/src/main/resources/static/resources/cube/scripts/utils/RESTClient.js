	...
	return function() {
		...
		me.send = function(p_path, p_method, p_data, p_async, p_callback) {
			...
			var ajaxSetting = {
				...
				cache : false,
				async : async,
				data : cube.encryptData ? cube.encryptData(p_data, url) : p_data,
			...
			if (cube.notEmpty(customHeaders)) {
				if (ajaxSetting.headers) {
					$.extend(ajaxSetting.headers, customHeaders);
				} else {
			...
			if (cube.notEmpty(cube.cl_u_id)) {
				if (ajaxSetting.headers) {
					$.extend(ajaxSetting.headers, {
						"cl_u_id": cube.cl_u_id
					});
				} else {
					ajaxSetting.headers = {
						"cl_u_id": cube.cl_u_id
		...
		/**
		 * 使用 HTTP POST 方法，以同步方式进行 REST 服务调用。
		 * 
		 * @param p_path
		 *            指定的 REST
		 *            服务路径，可以是以“~/”开头的绝对路径，如“~/rest/is/appsuites”；也可以是相对于
		 ...
		 *            指定传入的参数信息。通常是一个 JSON 对象。如 { id: 1984, name: "中文" }。
		 */
		me.postSync = function(p_path, p_data) {
			return me.sendSync(p_path, "POST", p_data);
		};
		...
		me._makeUrl = function(p_path) {
			...
			var token, arr, reg = new RegExp("(^| )token=([^;]*)(;|$)");
