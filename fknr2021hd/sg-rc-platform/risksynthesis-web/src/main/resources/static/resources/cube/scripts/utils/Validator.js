 ...
 * 		// 校验身份证码，给出提示。
 * 		var result = validator.validate("310212334466", "IDCARD");
 * 		if (!result.successful)
 * 		{
 * 		}
 * 
 * 		// 校验数字。
 * 		validator.validate(32, "DIGIT", { minValue: 10, maxValue: 50});
 * 
 * 		})
 * </code>
 */
...
define(["CheckUtil"], function(checkUtil) {
    ...
    me.validate = function(p_str, p_validType, p_options)
    {
    	var result = {
            successful: true,
            hint:  cube.msg("INVALID_MESSAGE")
        };
        
        var rules = [];
        var validTypes = [];
        
        if (cube.notEmpty(p_validType))
        {
        	if (cube.isString(p_validType))
        	{
    			validTypes = p_validType.split(",");
    			for (var i = 0; i < validTypes.length; i++) {
    				if (cube.notEmpty(validTypes[i])) {
    					var validType = validTypes[i].replace(/\s*/, "");
    					if (validType == "NOTNULL") {//首先对NOTNULL进行校验，通过之后再校验其他
    						_validateRules(p_str, [me.rules[validType]], result, validType, p_options);
    						if (!result.successful) {
    			                return result;
    			            }
    					} else {
    						rules.push(me.rules[validType]);
    					}
    				}
    			}
        	}
        ...
        // 当校验类型为单个时，执行该校验。
        if (cube.notEmpty(p_options) && cube.notEmpty(p_options.maxLength))
        {
        	$.extend(result, _isMaxLength(p_str, p_options.maxLength));
        	if (!result.successful)
            {
                if (cube.notEmpty(p_options.validateMessage))
                {
                    result.hint += _CRLF;
        ...
        if (cube.notEmpty(p_options) && cube.notEmpty(p_options.minLength))
        {
    ...
    function _validateRules(p_str, p_rules, p_result, p_validTypes, p_options)
    {
    	...
    	for (var i = 0; i < p_rules.length; i++) {
    		if (cube.notEmpty(p_options))
    		{
				ruleOption = p_options[p_validTypes[i]];    			
    		}

			if (cube.isEmpty(ruleOption))
			{
				ruleOption = p_options;
			}
			
			
			if (cube.notEmpty(ruleOption) && cube.notEmpty(ruleOption.maxLength))
	        {
				tmpRet1 = _isMaxLength(p_str, ruleOption.maxLength);
	        }
	        if (cube.notEmpty(ruleOption) && cube.notEmpty(ruleOption.minLength))
	        {
			...
			if (cube.notEmpty(tmpRet))
			{
				if (cube.notEmpty(tmpRet1) && !tmpRet1.successful)
				{
				...
				if (cube.notEmpty(tmpRet2) && !tmpRet2.successful) 
				{
				...
				if (!tmpRet.successful)
				{
					...
					if (cube.notEmpty(ruleOption) && cube.notEmpty(ruleOption.validateMessage))
					{
						p_result.hint += _CRLF;
						p_result.hint += ruleOption.validateMessage;
					}
					else if (cube.notEmpty(tmpRet.hint))
					{
						p_result.hint += _CRLF;
    ...
    {
		var　result = {
			successful: ! checkUtil.isNull(p_str)
    	};
    	if (!result.successful)
    	{
    		$.extend(result, {hint: cube.msg("INPUT_NON_NULLABLE")});
    	}
    	return result;
    }
    ...
    function _checkValue(p_result,p_str, p_options)
    {
		...
		if (cube.notEmpty(p_options))
		{
			if (cube.notEmpty(p_options.maxValue))
			{
				$.extend(result, _isMaxValue(p_str, p_options.maxValue, true));
				if (!result.successful)
					return result;
			}
			if (cube.notEmpty(p_options.minValue))
			{
    ...
    {
    	var　result = {
    		successful: checkUtil.isDigit(p_str)
    	};
    	if (result.successful)
    	{
    	...
    	{
    		$.extend(result, {hint: cube.msg("ENTER_DIGIT")});
    	}
    	return result;
    }
    ...
    {
    	var　result = {
    		successful: checkUtil.isInteger(p_str)
    	};
    	if (result.successful)
    	{
    		_checkValue(result, p_str, p_options);	
    	...
    	{
    		$.extend(result, {hint: cube.msg("ENTER_INTEGER")});
    	}
    	return result;
    }
    ...
    {
    	var　result = {
    		successful: checkUtil.isMinusInteger(p_str)
    	};
    	if (result.successful)
    	{
    		_checkValue(result, p_str, p_options);
    	...
    	{
    		$.extend(result, {hint: cube.msg("ENTER_MINUS_INTEGER")});
    	}
    	return result;
    }
    ...
    {
    	var　result = {
    		successful: checkUtil.isPlusInteger(p_str)
    	};
    	if (result.successful)
    	{
    		_checkValue(result, p_str, p_options);
    	...
    	{
    		$.extend(result, {hint: cube.msg("ENTER_PLUS_INTEGER")});
    	}
    	return result;
    }
    ...
    {
    	var　result = {
    		successful: checkUtil.isFloat(p_str)
    	};
    	if (result.successful)
    	{
    		_checkValue(result, p_str, p_options);
    	...
    	{
    		$.extend(result, {hint: cube.msg("ENTER_FLOAT")});
    	}
    	return result;
    }
    ...
    {
		var　result = {
		successful: checkUtil.isMinusFloat(p_str)
    	};
    	if (result.successful)
    	{
    		_checkValue(result, p_str, p_options);
    	...
    	{
    		$.extend(result, {hint: cube.msg("ENTER_MINUS_FLOAT")});
    	}
    	return result;
    }
    ...
    {
		var　result = {
			successful: checkUtil.isPlusFloat(p_str)
    	};
    	if (result.successful)
    	{
    		_checkValue(result, p_str, p_options);
    	...
    	{
    		$.extend(result, {hint: cube.msg("ENTER_PLUS_FLOAT")});
    	}
    	return result;
    }
    ...
    {
    	var result = {
    		successful: checkUtil.isMaxValue(p_str, p_maxValue, p_isFloat)
    	}
    	if (!result.successful)
    	{
    		$.extend(result, {hint: cube.msg("INPUT_GREATER_THAN_MAX", [p_maxValue])})
    	}
    	return result;
    }
    ...
    {
    	var result = {
    		successful: checkUtil.isMinValue(p_str, p_minValue, p_isFloat)
    	}
    	if (!result.successful)
    	{
    		$.extend(result, {hint: cube.msg("INPUT_LESS_THAN_MIN", [p_minValue])});
    	}
    	return result;
    }
    ...
    {
    	var result = {
    		successful: checkUtil.isMinLength(p_str, p_minLength - 1)
    	}
    	if (!result.successful)
    	{
    		$.extend(result, {hint: cube.msg("INPUT_LENGTH_LESS_THAN_MIN", [p_minLength])});
    	}
    	return result;
    }
    ...
    {
    	var result = {
    		successful: checkUtil.isMaxLength(p_str, p_maxLength + 1)
    	}
    	if (!result.successful)
    	{
    		$.extend(result, {hint: cube.msg("INPUT_LENGTH_GREATER_THAN_MAX", [p_maxLength])})
    	}
    	return result;
    }
    ...
    {
    	var　result = {
    		successful: checkUtil.isChinese(p_str)
    	};
    	if (!result.successful)
    	{
    		$.extend(result, {hint: cube.msg("ENTER_CHINESE")});
    	}
    	return result;
    }
    ...
    {
    	var　result = {
    		successful: checkUtil.isLetter(p_str)
    	};
    	if (!result.successful)
    	{
    		$.extend(result, {hint: cube.msg("ENTER_LETTER")});
    	}
    	return result;
    }
    ...
    {
    	var result = {
    		successful: checkUtil.isAcsii(p_str)
    	}
    	if (!result.successful)
    	{
    		$.extend(result, {hint: cube.msg("NON_ASCII")});
    	}
    	return result;
    }
    ...
    {
    	var result = {
    		successful: checkUtil.isDate(p_str)
    	}

    	if (!result.successful)
    	{
    		$.extend(result, {hint: cube.msg("ENTER_DATE")});
    	}
		else
    	{
    		if (cube.notEmpty(p_options))
    ...
    {
    	var result = {
    		successful: checkUtil.isDateTime(p_str)
    	}

    	if (!result.successful)
    	{
    		$.extend(result, {hint: cube.msg("ENTER_DATETIME")});
    	}
    	else
    	{
    		if (cube.notEmpty(p_options))
    ...
    {
    	var result = {
    		successful: checkUtil.isTime(p_str)
    	}
    	if (result.successful)
    	{
    	...
    	if (!result.successful)
    	{
    		$.extend(result, {hint: cube.msg("ENTER_TIME")});
    	}
    	return result;
    }
    ...
    function _checkTimeMinAndMax(p_str,p_options){
    	...
    	if (cube.notEmpty(p_options) && (cube.notEmpty(p_options.maxValue) || cube.notEmpty(p_options.minValue)))
		{	
			var date = new Date();
			date.setHours(0);
			date.setMinutes(0);
			...
			if (cube.notEmpty(p_options.maxValue))
			{
    			...
    			if(v>max){
 				   ...
 				   result.hint =cube.msg("INPUT_GREATER_THAN_MAX", [p_options.maxValue]);
 				   return result;
 				}
			}
			if (cube.notEmpty(p_options.minValue))
			{
				...
				if(v<min){
				   result.successful = false;
				   result.hint =cube.msg("INPUT_LESS_THAN_MIN", [p_options.minValue]);
				   return result;
				}
			}
		}
    ...
    {
    	var result = {
    		successful: checkUtil.isEmail(p_str)
    	}
    	if (!result.successful)
    	{
    		$.extend(result, {hint: cube.msg("ENTER_EMAIL")});
    	}
    	return result;
    }
    ...
    {
    	var result = {
    		successful: checkUtil.isIP(p_str)
    	}
    	if (!result.successful)
    	{
    		$.extend(result, {hint: cube.msg("ENTER_IP")});
    	}
    	return result;
    }
    ...
    {
    	var result = {
    		successful: checkUtil.isMobile(p_str)
    	}
    	if (!result.successful)
    	{
    		$.extend(result, {hint: cube.msg("ENTER_MOBILE")});
    	}
    	return result;
    }
    ...
    {
    	var result = {
    		successful: checkUtil.isPhone(p_str)
    	}
    	if (!result.successful)
    	{
    		$.extend(result, {hint: cube.msg("ENTER_PHONE")});
    	}
    	return result;
    }
    ...
    {
    	var result = {
    		successful: checkUtil.isUrl(p_str)
    	}
    	if (!result.successful)
    	{
    		$.extend(result, {hint: cube.msg("ENTER_URL")});
    	}
    	return result;
    }
    ...
    {
    	var result = {
    		successful: checkUtil.isZipCode(p_str)
    	}
    	if (!result.successful)
    	{
    		$.extend(result, {hint: cube.msg("ENTER_ZIPCODE")});
    	}
    	return result;
    }
   	...
   	{
		var result = checkUtil.isSqlInjection(p_str);
    	if (!result.successful)
    	{
    		$.extend(result, {hint: cube.msg("SQL_INJECTION", {sql: result.hint})});
    	}
    	return result;
   	}
   	...
   	{
   		var result = {
			successful: checkUtil.isIDCard(p_str)
    	}
    	if (!result.successful)
    	{
    		$.extend(result, {hint: cube.msg("ENTER_IDCARD")});
    	}
    	return result;
   	}
   	
	function _unique(p_str, p_options)
   	{
   		var result = {
			successful: checkUtil.isUnique(p_str, p_options.url )
    	}
    	if (!result.successful)
    	{
    		$.extend(result, {hint: p_options.validateMessage?p_options.validateMessage:cube.msg("ENTER_UNIQUE")});
    	}
    	return result;
   	}
     ...
     */
   	function _checkDateLimits(p_value, p_options)
   	{
   		...
   		if (cube.notEmpty(p_options.maxValue))
		{
			...
			if (!result.successful)
	    	{
	    		$.extend(result, {hint: cube.msg("INPUT_GREATER_THAN_MAX", [p_options.maxValue])})
	    	}
		}
		
		if (result.successful && cube.notEmpty(p_options.minValue))
		{
			try
			{
			...
			if (!result.successful)
	    	{
	    		$.extend(result, {hint: cube.msg("INPUT_LESS_THAN_MIN", [p_options.minValue])});
	    	}
		}
