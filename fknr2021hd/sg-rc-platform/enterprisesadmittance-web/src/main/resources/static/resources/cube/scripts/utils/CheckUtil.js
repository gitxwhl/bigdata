...
define(["RESTClient"], function(RESTClient) {
    ...
    {
        if (null == p_str || (cube.isString(p_str) && "" == p_str.replace(/^\s*/, "").replace(/\s*$/, "")))
        {
            return true;
        }
        else
    ...
    me.isDate = function(p_str)
    {
    	...
    	var parseDate = Date.parse(p_str);
    	if (p_str.length == 4 && !isNaN(parseDate)) {
    		return true;
    	}
        ...
        if(r[1] && r[1].length == 4) {
        	var md = r[1];
        	r[1] = md.substring(0, 2);
        	r[2] = md.substring(2, 4);
        }
    ...
    me.isDateTime = function(p_str)
    {
        ...
        {	 
             ...
             if(r.length <= 5)
             {
            	 ...
            	 //yyyyMMdd HH:mm:ss
            	 if (r.length == 5 && r[1].length == 4) {
            		 r[5] = r[4];
            		 r[4] = r[3];
            		 r[3] = r[2];
            		 r[2] = r[1].substring(2, 4);
            		 r[1] = r[1].substring(0, 2);
            	 } else if(r.length == 4 && r[1].length == 4) {
            		 r[4] = r[3];
            		 r[3] = r[2];
            		 r[2] = r[1].substring(2, 4);
            		 r[1] = r[1].substring(0, 2);
            	 } else if(r.length == 4 && r[0].length == 4 && r[1].length == 2 && r[2].length == 2) {
            		 r[4] = "00";
            	 } else {
            		 if (typeof(r[3]) != "undefined" && r[2].length == 4) {
                		 r[5] = r[3];
                	 }
                	 
                	 if (typeof(r[2]) != "undefined" && r[2].length == 4) {
                		 r[4] = r[2].substring(2, 4);
                		 r[3] = r[2].substring(0, 2);
                	 }

                	 if (typeof(r[1]) != "undefined" && r[1].length == 4) {
                		 r[2] = r[1].substring(2, 4);
                		 r[1] = r[1].substring(0, 2);
                	 }
            	 }
     ...
     * 
     * @param p_str, 一个字符串，表示待检查的字符串。
     * @param p_url, 一个验证地址，唯一时返回true，否则返回false。
     */
    me.isUnique = function(p_str, p_url)
    {
		if(p_str == null)
		{
			p_str = "";
    	...
    	restClient.async = false;
    	restClient.get(p_url,{"name": p_str},function(data) {
