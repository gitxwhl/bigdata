<template>
    ...
    <el-card class='tablecard'>
            ...
            <el-row class="iptrow">
                ...
                <el-col :span='2'  class='iptcol'>
                    <el-form-item>
                        <button class='search' @click.prevent >预定</button>
                    </el-form-item>
                </el-col>
            </el-row>
            </el-form>
        <el-table stripe ref="multipleTable" :data="tableData" :cell-style="{padding:'0',
            ...
            fontWeight:'normal'}" :header-cell-style="{padding:'0',color:'rgb(255,255,255)'}" tooltip-effect="dark"
             height="740px" size="small"  style="width: 100%;margin-top:0px;" @selection-change="handleSelectionChange">
                ...
                <el-table-column align="center"  min-width="12%" prop="ordernumber" label="订单编号" show-overflow-tooltip> </el-table-column>
                <el-table-column align="center"  min-width="12%"  prop="restaurant" label="下单餐厅" show-overflow-tooltip></el-table-column>
                <el-table-column align="center"  min-width="6%"  prop="ordertime" label="下单时间" show-overflow-tooltip></el-table-column>
                ...
                <el-table-column align="center"  min-width="6%"  prop="deduction" label="扣费情况" show-overflow-tooltip></el-table-column>
                <el-table-column align="center"  min-width="6%"  prop="personCnt" label="人员总数" show-overflow-tooltip></el-table-column>
                ...
                <el-table-column align="center"  min-width="30%"   label="操作" show-overflow-tooltip>
                    <template slot-scope="scope">
                        <span  class="editicon"><img src="../../assets/u4062.svg" alt=""  @click="dialogPackVisible=true"> 扣费失败名单</span>
                        <span  class="editicon"><img src="../../assets/u3655.svg" alt=""  @click="getorder(scope.row.ordernumber)"> 预定菜品详情</span>
                        <span  class="editicon"><img src="../../assets/u3656.svg" alt="" @click="cancelOrder(scope.row.id)"> 取消预订</span>
                    </template>
                </el-table-column>
        </el-table>
                    ...
                    <el-dialog class='dialog'  title="扣费失败人员名单" :visible.sync="dialogPackVisible">
                        <div class='placeres'>下单餐厅:</div>
                        <div class='placeres'>
                            <span>预定日期:</span>
                            <span>餐别类型:</span>
                            <span>取餐时间:</span>
                    ...
                    <div class="clearfix">
                        <!-- <button class='import' @click.prevent @click="reply">导出</button> -->
                        <button class='import' @click.prevent >导出</button>
                        <el-table  stripe ref="multipleTable" :data="tableDataname" :cell-style="{padding:'0',
                                color:'rgb(255,255,255)'}" :row-style="{height:'38px',lineHeight:'10px',fontSize:'13px',
                                fontWeight:'normal'}"  :header-row-style="{height:'38px',lineHeight:'10px',fontSize:'13px',
                                fontWeight:'normal'}" :header-cell-style="{padding:'0',color:'rgb(255,255,255)'}" tooltip-effect="dark"
                                 size="small"  style="width: 100%;margin-top:10px;" @selection-change="handleSelectionChange">
                                    <el-table-column align="center"  min-width="5%" type="selection" width="55"> </el-table-column>
                                    <el-table-column align="center"  min-width="10%" prop="name" label="姓名"> </el-table-column>
                                    <el-table-column align="center"  min-width="10%" prop="dishescategory" label="卡号"></el-table-column>
                                    <el-table-column align="center"  min-width="10%"  prop="weight" label="异常原因" show-overflow-tooltip></el-table-column>
                                </el-table>
                    </div>
                    ...
                    <el-dialog class='dialog'  title="生产现场订单/值班室订单/餐柜订单" :visible.sync="dialogNutVisible">
                        <div class='dialogttext'>
                            ...
                            <span>预定日期:{{order.scheduled}}</span>
                            <span>配送地址:{{order.address}}</span>
                            <span>联系电话:{{order.telephone}}</span>
                            <span>餐别类型:{{order.dictionary}}</span>
                            <span>取餐时间:{{order.picktime}}</span>
                        </div>
                <div class='amount'>合计：{{order.info.personCnt}}人</div>
                        <div class='peoplelist'>人员名单:{{order.info.personName}}</div>
                <div class='amount'>主食{{order.info.zsCnt}}份、凉菜{{order.info.lcCnt}}份、热菜{{order.info.rcCnt}}份、汤类{{order.info.tlCnt}}份、水果{{order.info.sgCnt}}份，合计：{{order.info.count}}份</div>
                            <div class='foodclassify'>主食:{{order.info.zs}}</div>
                            <div class='foodclassify'>凉菜:{{order.info.lc}}</div>
                            <div class='foodclassify'>热菜:{{order.info.rc}}</div>
                            <div class='foodclassify'>汤类:{{order.info.tl}}</div>
                            <div class='foodclassify'>水果:{{order.info.sg}}</div>
...
export default {
  ...
  methods: {
    ...
    pagelist(){
        this.$axios({
            url:'StOrdering.do/getOrderList',
            method:'post',
            data:{
