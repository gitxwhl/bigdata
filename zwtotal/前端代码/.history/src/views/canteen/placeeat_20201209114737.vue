<template>
    ...
    <el-card class='tablecard'>
        ...
        <div v-if="applyshow">
            ...
            <div class="applypeople">申请人：{{name}} </div>            
            ...
            <!-- <el-row class="iptrow">
                <el-col :span='14' class='iptcol'>
                    <el-form-item label="出差地点:">
                       <el-input class="place" v-model="queryform.place"></el-input>
                    </el-form-item>
                </el-col>
            </el-row> -->
            <el-row class='iptrow'>
              ...
              <el-col :span='5'>
                <el-form-item label="公司:">
                  <el-select  v-model="queryform.companyData">
                    <el-option  @click.native="gongsiSelect(item.ORGID)"  v-for="item in company" :key="item.ORGID" :label="item.ORGNAME" :value="item.ORGID">
                    </el-option>
                  </el-select>
                </el-form-item>
              </el-col>
              ...
              <!-- <el-col :span='5'>
                <el-form-item label="部门:">
                  <el-select v-model="queryform.restaurant">
             ...
             <el-col :span='5'>
              <el-form-item label="选择餐厅:" prop="restaurant">
                <el-select v-model="queryform.restaurant">
                  <el-option v-for="item in stOperationList" :key="item.id" :label="item.name" :value="item.id">
                  </el-option>
                </el-select>
              </el-form-item>
            </el-col>
         ...
         <el-row class='iptrow'>
             ...
             <div class="approvepeople">
               <el-form-item label="接待人:" prop="restaurant">
                <el-select v-model="queryform.approvedBy">
                     <el-option v-for="item in approvalBy" :key="item.id" :label="item.name" :value="item.id">
                    </el-option>
                </el-select>
                 </el-form-item>
               </div>
...
<!--            </el-row>-->
    ...
    <div v-if="examshow">
          ...
          <div v-for='item in approvelist' :key='index'>
            <el-form :model="queryform" ref="queryform" size="mini" class='queryform'>
            <div class="applypeople">申请人:{{item.AuditUserName}}</div>
            <div class="applypeople">申请时间： </div>
            <div class="applypeople">出差地点：</div>
            <div class="applypeople">选择餐厅：</div>
            <div class="applypeople">说明：</div>

            <div class="applypeople">审批人：</div>
             ...
             <el-row class='iptrow'>
                ...
                <button class='search' @click.prevent @click='approvecircs(item.id,2)'>退回</button>
              <button class='search' @click.prevent @click='approvecircs(item.id,3)'>同意</button>
                </el-col>
             </el-row>
            </el-form>
          </div>
      ...
      :total="totalsp">
        ...
        <div v-if="dealtshow">
          <el-row class="iptrow">
                ...
                <el-col :span='3' class='iptcol'>
                     <button class='search' @click.prevent @click="dealtchange(state)">检索</button>
                </el-col>
          </el-row>
          <div class='text'>
        ...
        <el-table-column align="center"  min-width="10%" prop="AuditUserName" label="申请人"> </el-table-column>
        <el-table-column align="center"  min-width="10%" prop="endTime" label="使用时段"></el-table-column>
        <el-table-column align="center"  min-width="12%"  prop="place" label="出差地点" show-overflow-tooltip></el-table-column>
...
export default {
  ...
  methods: {
    ...
    dealtchange(state){
        ...
        this.state=state
        this.$axios({
          url:'appTApplyCanteen.do/getAgencyListPage',
          method:'post',
          data:{
          ...
          this.total2=res.data.msg.totalRecord
          this.tableData=res.data.msg.list
          // this.pagexinxi=res.data
...
// 申请人查询保存的申请
    ...
    gongsi(){
      this.$axios({
        url:'appTApplyCanteen.do/getCompanyList',
        method:'post', 
        data:{}    
      }).then(res=>{
    ...
    gongsiSelect(ss){ 
      this.$axios({
       ...
       this.$axios({
          url:'common.do/getPersonList/' + ss,
          method:'POST',       
      }).then(res=>{       
        this.approvalBy = res.data.data
  ...
  //   },
    ...
    // stDeptList(){
    //   this.$axios({
    //     url:'common.do/getCompanyOrDeptList/2/101',
    //     method:'get',
    //   }).then(res=>{
    //     // console.log(res)
    //     this.department=res.data.data

    //     // this.approvalBy=res.data.approvalBy
    //   })
    // },
    ...
    addApplication(){
      ...
      // console.log(this.queryform.restaurant)
      // console.log(this.queryform.place)
      // console.log(this.queryform.receptionPersonName)
      ...
      this.queryform.restaurant==''||this.queryform.remarks==''|| this.queryform.approvedBy==this.queryform.receptionPersonName){
        ...
        return
          ...
          this.$axios({
                  url:'StApplyCanteenController.do/applyRemoteDining',
                  method:'post',
                  data:{
                    "applicant":1,
                    ...
                    "state":1,
                    "receptionPersonName":this.queryform.approvedBy,
                  }
                }).then(res=>{
                  if(res.data=="申请成功"){
    ...
    approvecircs(id,state){
      this.$axios({
        url:'StLocaleat.do/approval',
        method:'post',
        data:{
          "id":id,
          "explainInfo":this.approveremark,
          "state":state,
        }
      }).then(res=>{
        if(res.data=='已同意'){
