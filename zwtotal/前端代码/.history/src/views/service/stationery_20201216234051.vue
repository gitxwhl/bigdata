<template>
    ...
    <el-card class='tablecard'>
      <div v-show="applyforshow">
        <div class='imgbox'>
          ...
          <span  class="assess"><span class="imgdescribe">评价</span><img src="../../assets/u105.svg" alt="" class='waterdrop' @click="evaluate" style="transform: rotate(180deg)"></span>
            ...
            <el-form :model="queryform" ref="queryform" size="mini" class='queryform' >
              <el-row>
                ...
                <el-form-item label="申请原因:" >
                              ...
                              <el-row>
                                <el-col  :span="16" >
                                  <el-form-item  label="物品名称:">
                                    ...
                                    <i class="el-icon-plus" @click.prevent @click="oppoyssz"></i>
                                    </el-form-item>
                                </el-col> 
                                                       
                                        <el-dialog class="adddialog"  title="库存查询" :visible.sync="dialogaddVisible" @close="clear">
    
                                          <div class='mainbox'>
                                                  <div class='vforbox' v-for="(item,index) in list" :key="index">
                                                      <img :src='item.pictureurl' alt=""height="400" width="600" >
                                                      <div class='boxfather'>
                                                          <div class='smallbox'>
                                                      ...
                                                      <div class='goodsname clearfix'>
                                                          {{item.goodname}}
                                                          <div style='float:right;margin-right: 15px' class='el-icon-plus' @click="amend(item)"></div>
                                                      </div>
                                  ...
                                  <el-col :span="8">
                                      <el-form-item label="物品均价:">
                                        <el-input class='amountwidth' readonly v-model="item.unitprice"></el-input>
                                      </el-form-item>
                                    </el-col>
                              </el-row>
                        ...
                        <div class='time'>
                            <span>物品申请总数量:{{amount}}</span>
                            <span>物品申请总金额:<el-input  class='amountwidth' readonly v-model="money"></el-input></span>
                            <span>部门预算余额:</span>
                        </div>
                        <div  class='btnbox'>
      ...
      <div style="text-align: right; margin-top: 10px;height:'15%'">
        ...
        <el-table-column align="center" min-width="10%" label="详情" show-overflow-tooltip>
          <template slot-scope="scope">
            ...
            <div @click='lookdfftwo(scope.row)' v-show='scope.row.state=="已评价"'>
              查看详情
            </div>
      ...
      <div style="text-align: right; margin-top: 10px;height:'15%'">
        ...
        <!-- 评价 -->
        <div v-if='estimate' class='Initiateapplication'>
          <!-- <div  class="marginbtm">本人申请:审批</div>  -->
          <!-- <div  class="marginbtm">本人审批:</div>  -->
          <div class='time'>
            <span>待评价:{{dpj}}</span>
          </div>
          <el-table stripe ref="multipleTable" :data="estimatetableData" :cell-style="{padding:'0',
            color:'rgb(255,255,255)'}" :row-style="{height:'38px',lineHeight:'10px',fontSize:'13px',
            fontWeight:'normal'}"  :header-row-style="{height:'38px',lineHeight:'10px',fontSize:'13px',
            fontWeight:'normal'}" :header-cell-style="{padding:'0',color:'rgb(255,255,255)'}" tooltip-effect="dark"
                    size="small"  style="width: 100%;margin-top:0px;" >
      ...
      <div v-show="detailshow" class='detailbox'>
        ...
        <div class='clearfix colbox'>
          <div style="float:left;">
            <span>所在部门:{{dffrow.demanddep}}</span>
    ...
    <div v-show="detailtwoshow" class='detailbox'>
            ...
            <div class='clearfix colbox'>
              <div style="float:left;">
                <span>所在部门:{{dsprow.demanddep}}</span>
      ...
      <div v-show="grantshow" class='detailbox'>
            ...
            <div class='clearfix colbox'>
              <div style="float:left;">
                <span>所在部门:{{dfftworow.demanddep}}</span>
...
<!--                <button class='search' @click.prevent @click="fafang(dfftworow.orderid)">发放</button>-->
      ...
      <div v-show="granttwoshow" class='detailbox'>
        ...
        <div class='clearfix colbox'>
          <div style="float:left;">
            <span>所在部门:{{yfftworow.demanddep}}</span>
      ...
      <div v-show="ypjpage" class='detailbox'>
        ...
        <div style="float:right;margin-right: 80px"><button class='search' @click.prevent @click="publish">发表</button></div>
      </div>
    ...
    amount(){
      var o=Number(0)
       for(var i=0;i<this.queryform.goods.length;i++){
          o+=Number(this.queryform.goods[i].goodamount)
      }
      return o
    },
    money(){
      var b=Number(0)
       for(var i=0;i<this.queryform.goods.length;i++){
          b+=Number(this.queryform.goods[i].goodamount*this.queryform.goods[i].unitprice)
      }
      return b
    }
  },
 ...
 // 下拉框数据
    ...
    neckchange(){
      ...
      this.$office({
        ...
        data:{
          ...
          "typeid": "003",//类型 003：办公文具 	005：打印纸
          "depId":'',//部门，传空
    ...
    evaluate(){
      ...
      this.$office({
        ...
        this.estimatetableData=res.data.page.list
        this.estimatetotal=res.data.page.totalRecord
    ...
    //评价发表
    publish(row){
      // console.log(this.ypjrow.goods)
      this.$office({
        url:'TdeptBudget.do/evaluate',
        method:'post',
        data:[
          {
            "orderId": this.ypjrow.goods[0].relateorderid,		//编号，从列表中获取
    ...
    // 提交办公服务用品申请
    TapplyOrder(){
      var form={"amountPrice":this.money,...this.queryform}
      if(this.queryform.receiptdate==''||this.queryform.applyphone==''||this.queryform.remark==''){
        this.$message('请补全信息')
        return
      }
      for(var i=0;i<this.queryform.goods.length;i++){
