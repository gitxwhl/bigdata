...
public class ExportExcelPlus<T>  {
    ...
    public void export(HttpServletResponse response, String fileName, Map map, Map map1) throws UnsupportedEncodingException {
...
//        comment.setAuthor("yang");
        List<String> listfast = (List<String>)map.get("listfast");

        List<Map> listfast1 = (List<Map>)map1.get("listfast1");
        List<Map> listlunch1 = (List<Map>)map1.get("listlunch1");
        List<Map> listdinner1 = (List<Map>)map1.get("listdinner1");

        int len = listfast.size();
        // 产生表格标题行
        HSSFRow row = sheet.createRow(0);
        for (short i = 0; i < listfast.size(); i++)
        {
            HSSFCell cell = row.createCell(i);
            cell.setCellStyle(style);
            HSSFRichTextString text = new HSSFRichTextString(listfast.get(i));
            cell.setCellValue(text);
        }
        for(int i=0; i<listfast1.size(); i++){
            row=sheet.createRow(i+1);
            Map vpd = listfast1.get(i);
            for(int j=0;j<len;j++){
        ...
        for(int i=0; i<listlunch1.size(); i++){
            row=sheet.createRow(i+2+b);
            Map vpd = listlunch1.get(i);
            for(int j=0;j<len;j++){
                String varstr = vpd.get("var"+(j+1)) != null ? String.valueOf(vpd.get("var"+(j+1))) : "";
                row.createCell(j).setCellValue(varstr);
            }
        ...
        for(int i=0; i<listdinner1.size(); i++){
            row=sheet.createRow(i+2+c);
            Map vpd = listdinner1.get(i);
            for(int j=0;j<len;j++){
                String varstr = vpd.get("var"+(j+1)) != null ? String.valueOf(vpd.get("var"+(j+1))) : "";
                row.createCell(j).setCellValue(varstr);
            }
